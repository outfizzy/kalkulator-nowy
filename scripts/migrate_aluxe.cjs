    ...
    for (let r = 0; r < Math.min(20, json.length); r++) {
        ...
        if (rowStr.includes('maß') || rowStr.includes('bxt')) {
            ...
            row.forEach((cell, idx) => {
                ...
                else if (c.includes('ir-gold') || c.includes('ir gold') || c.includes('aufpreis ir')) irGoldColIdx = idx;
                else if (c.includes('matt') || c.includes('milch') || c.includes('aufpreis matt')) mattColIdx = idx;
