...
const STATUS_STYLES: Record<B2BOrderStatus, { color: string; icon: string; step: number; label: string }> = {
    pending: { color: 'bg-yellow-100 text-yellow-800 border-yellow-300', icon: '⏳', step: 1, label: 'Oczekujące' },
    approved: { color: 'bg-blue-100 text-blue-800 border-blue-300', icon: '✅', step: 2, label: 'Zatwierdzone' },
    rejected: { color: 'bg-red-100 text-red-800 border-red-300', icon: '❌', step: -1, label: 'Odrzucone' },
    awaiting_payment: { color: 'bg-orange-100 text-orange-800 border-orange-300', icon: '💳', step: 2, label: 'Oczekiwanie na płatność' },
    in_production: { color: 'bg-purple-100 text-purple-800 border-purple-300', icon: '🏭', step: 3, label: 'W produkcji' },
    shipped: { color: 'bg-indigo-100 text-indigo-800 border-indigo-300', icon: '🚚', step: 4, label: 'Wysłane' },
    delivered: { color: 'bg-green-100 text-green-800 border-green-300', icon: '📦', step: 5, label: 'Dostarczone' },
    cancelled: { color: 'bg-gray-100 text-gray-600 border-gray-300', icon: '🚫', step: -1, label: 'Anulowane' }
};
...
export function B2BOrdersPage() {
    ...
    const progressSteps = [
        { step: 1, label: 'Zamówione', icon: '📋' },
        { step: 2, label: 'Zatwierdzone', icon: '✅' },
        { step: 3, label: 'Produkcja', icon: '🏭' },
        { step: 4, label: 'Wysyłka', icon: '🚚' },
        { step: 5, label: 'Dostarczone', icon: '📦' }
    ];
    ...
    return (
        <div className="p-6 max-w-[1600px] mx-auto">
            ...
            <div className="mb-6">
                ...
                <p className="text-gray-500 mt-1">Śledź status swoich zamówień</p>
            </div>
            ...
            <div className="flex gap-4 mb-6">
                <div className="flex bg-gray-100 rounded-lg p-1">
                    {(['active', 'completed', 'all'] as FilterType[]).map(f => (
                        ...
                        >
                            {f === 'all' && `Wszystkie (${counts.all})`}
                            {f === 'active' && `Aktywne (${counts.active})`}
                            {f === 'completed' && `Zakończone (${counts.completed})`}
                        </button>
                    ))}
                </div>
            </div>
                    ...
                    <div className="col-span-12 lg:col-span-8 bg-white rounded-xl shadow-sm border min-h-[700px]">
                        {selectedOrder ? (
                            <div className="p-6">
                                ...
                                <div className="flex justify-between items-start mb-6">
                                    <div>
                                        ...
                                        <span className={`inline-block mt-2 px-3 py-1 rounded-full text-sm font-medium border ${STATUS_STYLES[selectedOrder.status]?.color}`}>
                                            {STATUS_STYLES[selectedOrder.status]?.icon} {STATUS_STYLES[selectedOrder.status]?.label}
                                        </span>
                                    </div>
                                    <div className="text-right">
                                        ...
                                        <div className="text-sm text-gray-500">
                                            Zamówiono: {format(new Date(selectedOrder.created_at), 'dd.MM.yyyy HH:mm')}
                                        </div>
                                    </div>
                                </div>
                                ...
                                {!['rejected', 'cancelled'].includes(selectedOrder.status) && (
                                    <div className="mb-8 p-6 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl border border-blue-200">
                                        <h3 className="font-semibold text-blue-900 mb-4">📍 Status zamówienia</h3>
                                        <div className="flex justify-between relative">
                                            ...
                                            <div
                                                ...
                                                style={{
                                                    width: `calc(${((STATUS_STYLES[selectedOrder.status]?.step || 1) - 1) / 4 * 100}% - 4rem)`
                                                }}
                                            ></div>

                                            {progressSteps.map((step, idx) => {
                                                const currentStep = STATUS_STYLES[selectedOrder.status]?.step || 0;
                                ...
                                {selectedOrder.status === 'rejected' && selectedOrder.rejection_reason && (
                                    <div className="mb-6 p-4 bg-red-50 rounded-xl border border-red-200">
                                        <h3 className="font-semibold text-red-800 mb-2">❌ Zamówienie odrzucone</h3>
                                ...
                                {selectedOrder.prepayment_status === 'pending' && (
                                    <div className="mb-6 p-4 bg-orange-50 rounded-xl border border-orange-200">
                                        <h3 className="font-semibold text-orange-800 mb-2">💳 Wymagana zaliczka</h3>
                                        <p className="text-orange-700">
                                            Prosimy o przelew <b>€{selectedOrder.prepayment_amount.toLocaleString()}</b>, aby rozpocząć realizację zamówienia.
                                        </p>
                                        <p className="text-sm text-orange-600 mt-2">
                                            Produkcja rozpocznie się po wpływie płatności.
                                        </p>
                                    </div>
                                )}
                                ...
                                {selectedOrder.tracking_number && (
                                    <div className="mb-6 p-4 bg-indigo-50 rounded-xl border border-indigo-200">
                                        <h3 className="font-semibold text-indigo-800 mb-2">🚚 Śledzenie przesyłki</h3>
                                ...
                                {selectedOrder.estimated_delivery && (
                                    <div className="mb-6 p-4 bg-green-50 rounded-xl border border-green-200">
                                        <h3 className="font-semibold text-green-800 mb-2">📅 Przewidywana dostawa</h3>
                                        <p className="text-lg text-green-900">
                                            {format(new Date(selectedOrder.estimated_delivery), 'EEEE, dd. MMMM yyyy', { locale: pl })}
                                        </p>
                                    </div>
                                )}
                                ...
                                <div className="grid grid-cols-2 gap-6 mb-6">
                                    <div className="p-4 bg-gray-50 rounded-lg">
                                        ...
                                        {selectedOrder.shipping_address ? (
                                            ...
                                            <p className="text-gray-400 text-sm">Brak podanego adresu</p>
                                        )}
                                    </div>
                                    <div className="p-4 bg-gray-50 rounded-lg">
                                        <h4 className="font-medium text-gray-700 mb-2">💰 Status płatności</h4>
                                        <div className="text-sm">
                                            {selectedOrder.prepayment_status === 'not_required' && (
                                                <span className="text-gray-600">Zaliczka nie jest wymagana</span>
                                            )}
                                            {selectedOrder.prepayment_status === 'pending' && (
                                                <span className="text-orange-600 font-medium">
                                                    Zaliczka oczekuje: €{selectedOrder.prepayment_amount.toLocaleString()}
                                                </span>
                                            )}
                                            {selectedOrder.prepayment_status === 'paid' && (
                                                <span className="text-green-600 font-medium">✓ Zaliczka opłacona</span>
                                            )}
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="border-t pt-6">
                                    ...
                                    {selectedOrder.timeline?.length ? (
                                            ...
                                            {selectedOrder.timeline.map((entry, idx) => (
                                                <div key={entry.id} className="flex gap-4">
                                                    <div className="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center text-lg flex-shrink-0">
                                                        {STATUS_STYLES[entry.status as B2BOrderStatus]?.icon || '•'}
                                                    </div>
                                        ...
                                        <p className="text-gray-400 text-sm">Brak wpisów historii</p>
                                    )}
                                </div>
                            </div>
                        ) : (
                            <div className="flex flex-col items-center justify-center h-full text-gray-400 py-32">
                                ...
                                <p>Kliknij na zamówienie, aby zobaczyć szczegóły</p>
                            </div>
                        )}
                    </div>
                </div>
