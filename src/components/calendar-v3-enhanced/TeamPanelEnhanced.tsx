...
export const TeamPanelEnhanced: React.FC<TeamPanelEnhancedProps> = ({
    ...
    const teamWorkload = useMemo(() => {
        ...
        teams.forEach(team => {
            ...
            const teamUnavail = weekDays.filter(day => 
                                ...
                                <div className="mb-2">
                                    ...
                                    <div className="flex flex-wrap gap-1">
                                        {team.members.map((member, idx) => (
                                            <span
                                                key={member.id || idx}
                                                className={`px-2 py-0.5 text-xs rounded flex items-center gap-0.5 ${
                                                    member.id === team.leaderId
                                                        ? 'bg-amber-100 text-amber-800 font-bold'
                                                        : 'bg-slate-100 text-slate-700'
                                                }`}
                                            >
                                                {member.id === team.leaderId && <Crown className="w-2.5 h-2.5" />}
