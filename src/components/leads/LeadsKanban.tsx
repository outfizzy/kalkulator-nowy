...
export const LeadsKanban: React.FC<LeadsKanbanProps> = ({ leads, onLeadUpdate }) => {
    ...
    // Only forward direction — never reverse (leads in 'formularz' stay there)
    const autoMoveRunningRef = useRef(false);
    const autoMoveDoneRef = useRef(new Set<string>());
    useEffect(() => {
        ...
        const bulkMove = async () => {
            ...
            if (error) {
                ...
                forwardIds.forEach(id => autoMoveDoneRef.current.add(id));
