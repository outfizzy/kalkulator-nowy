...
export const CustomerDetails: React.FC<CustomerDetailsProps> = ({ customer, onEdit, onBack }) => {
    ...
    const [activeTab, setActiveTab] = useState<'overview' | 'financials' | 'logistics' | 'communication' | 'profitability' | 'leads' | 'measurements'>('overview');
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden min-h-[500px]">
                <div className="border-b border-slate-200 flex">
                    {[
                        ...
                        { id: 'measurements', label: 'Pomiary Techniczne', icon: 'M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10' },
                        ...
                        <div className="space-y-6">
                            <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 mb-6">
                                <h3 className="text-sm font-bold text-blue-800 mb-1 flex items-center gap-2">
                                    ...
                                    Moduł Pomiarów Technicznych
                                </h3>
                                <p className="text-sm text-blue-700">
                                    Tutaj możesz tworzyć techniczne pomiary konstrukcji (Dachrechner) przypisane do tego klienta.
                                    Pomiary te można później wykorzystać przy tworzeniu ofert lub kontraktów.
                                </p>
                            </div>

                            <ProjectMeasurementsList customerId={customer.id} />
                        </div>
                    )}
                                            ...
                                            <tr key={offer.id} className="hover:bg-slate-50 transition-colors">
                                                ...
                                                <td className="p-3">
                                                    ...
                                                    <div className="text-xs text-slate-400">
                                                        {formatMoney((offer.pricing.finalPriceNet || 0) * (((r) => r ? (r < 1 ? 1 + r : r) : ((offer.pricing as any)?.currency === 'PLN' ? 1.23 : 1.19))((offer.pricing as any)?.vatRate)))} Brutto
                                                    </div>
                                                </td>
                                                <td className="p-3">
                                    ...
                                    {contracts.map(contract => (
                                        <tr key={contract.id} className="hover:bg-slate-50 transition-colors">
                                            ...
                                            <td className="p-3">
                                                ...
                                                <div className="text-xs text-slate-400">
                                                    {formatMoney((contract.pricing.finalPriceNet || contract.pricing.sellingPriceNet || 0) * (((r) => r ? (r < 1 ? 1 + r : r) : ((contract.pricing as any)?.currency === 'PLN' ? 1.23 : 1.19))((contract.pricing as any)?.vatRate)))} Brutto
                                                </div>
                                            </td>
