...
type ProductType = 'roof' | 'awning' | 'carport' | 'wintergarden' | 'joinery' | 'other';
type RoofType = 'slope' | 'flat';
type GlassType = 'panoramic' | 'framed' | 'none';
...
export const FairLeadForm: React.FC<FairLeadFormProps> = ({ fairId, onSaved }) => {
    ...
    // Configuration
    const [config, setConfig] = useState<LeadConfig>({ type: 'roof' });
    const [width, setWidth] = useState<string>('4000');
    const [projection, setProjection] = useState<string>('3000');
