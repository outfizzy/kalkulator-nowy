        ...
        <div className="border-b border-slate-100 last:border-0">
            ...
            <div className="flex items-center gap-3 py-3 px-1">
                ...
                <div className="flex-1 min-w-0">
                    <p className="font-semibold text-slate-800 text-sm leading-snug">{toCustomerLabel(item.name)}</p>
                    {item.quantity > 1 && (
                        <p className="text-xs text-[#1E6FD9] font-semibold mt-0.5">Menge: {item.quantity} Stk.</p>
                    )}
                </div>
...
const GERMAN_LABELS: Record<string, { label: string; description?: string; iconKey: string; category: string }> = {
    ...
    'topstyle xl': { label: 'Topstyle XL Terrassenüberdachung', description: 'Modulare Bauweise — Breite unbegrenzt', iconKey: 'structure', category: 'base' },
    'topline': { label: 'Topstyle Terrassenüberdachung', description: 'Kräftige Konstruktion', iconKey: 'structure', category: 'base' },
    'topline xl': { label: 'Topstyle XL Terrassenüberdachung', description: 'Modulare Bauweise — Breite unbegrenzt', iconKey: 'structure', category: 'base' },
...
export const OfferSpecification: React.FC<OfferSpecificationProps> = ({ product, pricing }) => {
    ...
    if (product.customItems?.length) {
        for (const item of product.customItems) {
            allPositions.push({ label: toCustomerLabel(item.name), description: item.description || '', qty: item.quantity });
        }
    }
    ...
    return (
        <div className="bg-white rounded-2xl shadow-card border border-[#E5E7EB] overflow-hidden">
                ...
                <div className="flex items-start justify-between gap-4">
                    <div>
                        ...
                        <p className="text-sm text-slate-400 mt-1">
                            Alle Positionen im Angebotspreis enthalten
                        </p>
                    </div>
                    <span className="shrink-0 px-3 py-1.5 bg-emerald-50 border border-emerald-200 rounded-lg text-xs font-bold text-emerald-700 flex items-center gap-1.5">
                        <IconCheck className="w-3.5 h-3.5" />
                        {allPositions.length} Positionen
                    </span>
                </div>
            </div>
            ...
            <div className="divide-y divide-slate-100">
                {allPositions.map((pos, idx) => {
                    ...
                    return (
                        <div
                            key={idx}
                            className={`flex items-start gap-4 px-6 py-4 md:px-8 md:py-5 transition-colors ${
                                isBase ? 'bg-slate-50/80' : 'hover:bg-slate-50/50'
                            }`}
                        >
                            {/* Position number */}
                            ...
                            <div className="flex-1 min-w-0">
                                <p className={`font-bold leading-snug ${
                                    isBase ? 'text-base text-slate-900' : 'text-[15px] text-slate-800'
                                }`}>
                                    {pos.label}
                                </p>
                                {pos.description && (
                                    <p className="text-sm text-slate-400 mt-1 leading-relaxed">{pos.description}</p>
                                )}
                                {pos.config && (
                                    <p className="text-sm text-slate-400 mt-0.5">{pos.config}</p>
                                )}
                                {pos.qty && pos.qty > 1 && (
                                    <span className="inline-block mt-1.5 px-2 py-0.5 bg-[#EAF2FE] text-[#1E6FD9] text-xs font-bold rounded-md">
                                        Menge: {pos.qty} Stk.
                                    </span>
                                )}
                            </div>
                            ...
                            <div className="shrink-0 mt-1">
                                <span className="flex items-center gap-1 px-2.5 py-1 bg-emerald-50 rounded-lg border border-emerald-100">
                                    <IconCheck className="w-3.5 h-3.5 text-emerald-500" />
                                    <span className="text-[11px] font-bold text-emerald-600 uppercase tracking-wide">Inkl.</span>
                                </span>
                            </div>
                        </div>
                    );
            ...
            {/* ═══ FOOTER ═══ */}
            <div className="px-6 py-4 md:px-8 bg-slate-50 border-t border-slate-100 flex items-center gap-3">
                ...
                <p className="text-xs text-slate-400">
                    Alle aufgeführten Komponenten sind hochwertige Markenprodukte mit Herstellergarantie.
                </p>
            </div>
        </div>
    );
